...
- `math.rs`: inference activation kernels. On x86_64, `silu_and_mul_inplace` and `sigmoid_mul_inplace` dispatch to AVX-2 (8-wide) or AVX-512 (16-wide) SIMD implementations using range-reduced Taylor series exp approximation (same polynomial order as aarch64 NEON). Includes Newton-Raphson sigmoid refinement for ~23-bit accuracy. `rmsnorm` and `rmsnorm_inplace` dispatch to AVX-2 (8-wide) or AVX-512 (16-wide) SIMD paths with 2×8/16-wide FMA for sum-of-squares, matching aarch64 NEON vectorization. `softmax` dispatch to AVX-2 (8-wide) or AVX-512 (16-wide) with vectorized max scan, range-reduced polynomial exp, and normalized multiply, matching aarch64 NEON vectorization.
